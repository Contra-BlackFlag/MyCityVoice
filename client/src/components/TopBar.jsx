...
const TITLES = { "/": "CivicPulse", "/map": "Live Map", "/leaderboard": "Leaderboard", "/profile": "My Profile" };

export default function TopBar({ onCreateClick }) {
  const { pathname }     = useLocation();
  const { connected }    = useSocket();
  const navigate         = useNavigate();
  const title            = TITLES[pathname] || "CivicPulse";
  ...
  return (
    <header className="topbar">
      ...
      <div className="topbar-right">
        <div className={`live-chip ${connected ? "on" : "off"}`}>
          <span className="live-dot" />
          <span>{connected ? "Live" : "Off"}</span>
        </div>
        {pathname === "/" && (
          <button className="topbar-add" onClick={onCreateClick}>
