...
export default function PostCard({ report: init, isNew = false, onClick }) {
  ...
  const progress    = Math.min(((r.unique_upvoters || 0) / 5) * 100, 100);
  const isPinned    = r.pinned_to_map === 1;
  ...
  const vote = async (e, type) => {
    ...
    setBusy(true);
    try {
      const res = await reportsApi.vote(r.id, type);
      setR(p => ({ ...p,
        net_votes:       res.net_votes,
        upvote_count:    res.upvote_count,
        downvote_count:  res.downvote_count,
        unique_upvoters: res.unique_upvoters,
        pinned_to_map:   res.pinned_to_map,
        status:          res.status,
    ...
    <article className={`pc ${isNew ? "pc-new" : ""}`}>
      ...
      <div className="pc-head" onClick={() => navigate(`/profile/${r.user_id}`)}>
        ...
        <div className="pc-head-info">
          <div className="pc-username">
            ...
            {r.author?.badge && <span className="pc-badge">{r.author.badge}</span>}
          </div>
        ...
        {isPinned && (
          <div className="pc-pinned">
            <svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2C7.03 2 3 6.03 3 11c0 6.25 9 13 9 13s9-6.75 9-13c0-4.97-4.03-9-9-9z"/>
            </svg>
            Pinned
          </div>
        )}
      ...
      {r.image_url && (
        <div className="pc-img-wrap" onClick={() => onClick?.(r.id)}>
          <img src={r.image_url} alt={r.title} className="pc-img" loading="lazy"
            onError={e => e.target.parentElement.style.display = "none"} />
          <div className="pc-img-status">
      ...
      <div className="pc-body" onClick={() => onClick?.(r.id)}>
        {!r.image_url && (
          <span className="pc-status-chip pc-status-inline"
            style={{ color: status.color, background: `${status.color}22`, border: `1px solid ${status.color}44` }}>
            {status.label}
          </span>
        )}
      ...
      {!isPinned && (
        <div className="pc-progress-wrap">
          ...
          <span className="pc-progress-label">
            {r.unique_upvoters || 0}/5 unique upvotes to pin on map
          </span>
        </div>
      )}
      ...
      <div className="pc-actions">
        <div className="pc-votes">
          {/* Upvote */}
          <button
            className={`vote-btn up ${r.userVote === "up" ? "voted" : ""} ${isOwn ? "own" : ""}`}
            onClick={e => vote(e, "up")} disabled={busy || isOwn}
            title={isOwn ? "Cannot vote your own report" : "Upvote"}>
            <svg width="16" height="16" viewBox="0 0 24 24"
              fill={r.userVote === "up" ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.2">
          ...
          {/* Net score */}
          <div className={`net-score ${(r.net_votes||0)>0?"pos":(r.net_votes||0)<0?"neg":""}`}>
            {r.net_votes || 0}
          </div>

          {/* Downvote */}
          <button
            className={`vote-btn dn ${r.userVote === "down" ? "voted" : ""} ${isOwn ? "own" : ""}`}
            onClick={e => vote(e, "down")} disabled={busy || isOwn}
            title={isOwn ? "Cannot vote your own report" : "Downvote"}>
            <svg width="16" height="16" viewBox="0 0 24 24"
              fill={r.userVote === "down" ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2.2">
