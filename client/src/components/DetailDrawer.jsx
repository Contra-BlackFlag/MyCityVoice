...
export default function DetailDrawer({reportId,onClose}){
  ...
  const postComment=async()=>{
    ...
    setPosting(true);
    try{
      const res=await reportsApi.comment(report.id,comment.trim());
      setReport(p=>({...p,comments:[...(p.comments||[]),res.data],comment_count:(p.comment_count||0)+1}));
      setComment("");
  ...
  const deleteComment=async(cid)=>{
    ...
    try{
      await reportsApi.deleteComment(report.id,cid);
      setReport(p=>({...p,comments:(p.comments||[]).filter(c=>c.id!==cid),comment_count:Math.max(0,(p.comment_count||1)-1)}));
          ...
          <div className="dd-content">
            ...
            <div className="dd-body">
              ...
              <div className="dd-stats">
                ...
                <span>👥 {report.unique_upvoters||0} unique</span>
                <span>💬 {report.comment_count||0}</span>
                <span>🕐 {timeAgo(report.created_at)}</span>
              </div>
              <div className="dd-author" onClick={()=>{onClose();navigate(`/profile/${report.user_id}`);}}>
                <Avatar user={report.author} size={34}/>
                <div>
                  <div className="dd-author-name">@{report.author?.username} {report.author?.badge}</div>
                  <div className="dd-author-sub">Reporter · tap to view profile</div>
                </div>
              </div>
              ...
              <div className="dd-comments">
                ...
                <div className="dd-comment-row">
                  <input className="dd-comment-input" placeholder="Add a comment…" value={comment}
                    onChange={e=>setComment(e.target.value)} onKeyDown={e=>e.key==="Enter"&&postComment()} maxLength={500}/>
                  <button className="dd-comment-post" onClick={postComment} disabled={!comment.trim()||posting}>
                    {posting?"…":"Post"}
                  </button>
                </div>
                <div className="dd-comment-list">
                  {(report.comments||[]).length===0
                    ?<p className="dd-no-comments">No comments yet — be first!</p>
                    :[...(report.comments||[])].reverse().map(c=>(
                      <div key={c.id} className="dd-comment">
                        <Avatar user={{username:c.username,avatar_color:c.avatar_color,avatar_url:c.avatar_url}} size={28}/>
                        <div className="dd-comment-body">
                          <div className="dd-comment-header">
                            <span className="dd-comment-who">@{c.username} {c.badge}</span>
                            <span className="dd-comment-time">{timeAgo(c.created_at)}</span>
                          </div>
                          <p className="dd-comment-text">{c.content}</p>
                        </div>
                        {(user?.id===c.user_id||user?.role==="admin")&&(
                          <button className="dd-comment-del" onClick={()=>deleteComment(c.id)} title="Delete comment">
