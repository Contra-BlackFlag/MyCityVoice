...
function FlyTo({pos}){const map=useMap();useEffect(()=>{if(pos)map.flyTo(pos,15,{duration:1.2});},[pos,map]);return null;}
export default function MapPage(){
  ...
  useEffect(()=>{reportsApi.getMap().then(r=>setPins(r.data)).catch(console.error).finally(()=>setLoading(false));},[]);
  useEffect(()=>{
    if(!socket)return;
    const onNew=r=>{if(r.pinned_to_map){setPins(p=>[r,...p]);setFlyTo([r.latitude,r.longitude]);}};
    const onVote=({reportId,pinned_to_map,...data})=>{
      if(pinned_to_map)setPins(p=>{const e=p.find(r=>r.id===reportId);return e?p.map(r=>r.id===reportId?{...r,...data,pinned_to_map}:r):p;});
      else setPins(p=>p.filter(r=>r.id!==reportId));
    };
    socket.on("new_report",onNew);socket.on("vote_update",onVote);
    return()=>{socket.off("new_report",onNew);socket.off("vote_update",onVote);};
  },[socket]);
  return(
    <div className="mp">
      <div className="mp-bar"><span className="mp-count">📍 {pins.length} verified issue{pins.length!==1?"s":""}</span><span className="mp-hint">Community verified reports</span></div>
      <div className="mp-wrap">
        ...
        <MapContainer center={[20.5937,78.9629]} zoom={5} className="mp-leaflet" zoomControl>
          ...
          {pins.map(pin=>(
            <Marker key={pin.id} position={[pin.latitude,pin.longitude]} icon={makeIcon(pin.category)} eventHandlers={{click:()=>setSelectedId(pin.id)}}>
              <Popup>
                <div className="mp-popup">
                  ...
                  <div className="mp-popup-meta"><span>▲ {pin.net_votes||0}</span><span>{timeAgo(pin.created_at)}</span></div>
