...
export default function ProfilePage() {
  ...
  const score = profile
    ? profile.verified_count*10 + profile.total_upvotes*2 + profile.report_count
    : 0;
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="pp-breakdown">
            <div className="pp-bk-item">
              <span className="pp-bk-val">{profile.report_count * 1}</span>
              <span className="pp-bk-lbl">from reports</span>
            </div>
            <div className="pp-bk-sep"/>
            <div className="pp-bk-item">
              <span className="pp-bk-val">{profile.total_upvotes * 2}</span>
              <span className="pp-bk-lbl">from upvotes</span>
            </div>
            <div className="pp-bk-sep"/>
            <div className="pp-bk-item">
              <span className="pp-bk-val">{profile.verified_count * 10}</span>
              <span className="pp-bk-lbl">from verified</span>
            </div>
          </div>
          ...
          {posts.length === 0 ? (
            <div className="pp-empty">
              ...
              <p>{isOwn ? "You haven't posted anything yet." : "No reports yet."}</p>
            </div>
          ) : (
            posts.map(p => (
              <PostCard key={p.id} report={p} onClick={setSelectedId}/>
            ))
          )}
        </>
      ) : (
