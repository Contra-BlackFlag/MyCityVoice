...
export default function AdminSettings(){
  ...
  const save=async()=>{
    ...
    try{await adminApi.saveSettings({pin_threshold:threshold,area_name:areaName});setSaved(true);setTimeout(()=>setSaved(false),3000);}
  ...
  return(
    <div>
      ...
      <div className="adm-card">
        ...
        <input className="adm-input" value={areaName} onChange={e=>setAreaName(e.target.value)} placeholder="e.g. Nashik Zone 1"/>
      ...
      <div className="adm-card">
        ...
        <div className="adm-slider-wrap">
          <div className="adm-threshold-val">{threshold}</div>
          <input type="range" min="1" max="20" value={threshold} onChange={e=>setThreshold(parseInt(e.target.value))} className="adm-slider"/>
        </div>
        <p className="adm-hint">A report needs <strong>{threshold} unique upvote{threshold!==1?"s":""}</strong> to be pinned on the public map. This updates live for all users.</p>
      </div>
