...
export default function FeedPage() {
  ...
  useEffect(() => {
    if (!socket) return;
    const h = ({ reportId, ...data }) => {
      setPosts(p => p.map(r => r.id===reportId ? {...r,...data} : r));
    };
  ...
  return (
    <div className="fp">
      ...
      <div className="fp-filters">
        <div className="fp-scroll">
          {[{value:"newest",label:"🕐 New"},{value:"popular",label:"🔥 Hot"},{value:"oldest",label:"📅 Old"}].map(s=>(
            <button key={s.value}
              className={`fchip ${filter.sort===s.value?"active":""}`}
              onClick={()=>setFilter(p=>({...p,sort:s.value}))}>
              {s.label}
            </button>
          ))}
          <div className="fchip-sep"/>
          <button className={`fchip ${filter.category==="all"?"active":""}`}
            onClick={()=>setFilter(p=>({...p,category:"all"}))}>All</button>
          {CATEGORIES.map(c=>(
            <button key={c.value}
              className={`fchip ${filter.category===c.value?"active":""}`}
              style={{"--cc":c.color}}
              onClick={()=>setFilter(p=>({...p,category:c.value}))}>
              {c.icon} {c.label}
            </button>
          ))}
        </div>
      ...
      {/* New posts banner */}
      {newCount > 0 && (
        <button className="fp-new-banner"
      ...
      {loading ? (
        ...
        <>
          {posts.map(p=>(
            <PostCard key={p.id} report={p} isNew={newIds.has(p.id)} onClick={setSelectedId}/>
          ))}
          {posts.length < total && (
