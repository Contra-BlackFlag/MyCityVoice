...
async function recalculateRankings() {
  try {
    const { data: users } = await supabase
      .from("users")
      .select("id, report_count, verified_count, total_upvotes")
    ...
    const scored = users
      .map(u => ({ ...u, score: u.verified_count * 10 + u.total_upvotes * 2 + u.report_count }))
      .sort((a, b) => b.score - a.score)
      .slice(0, 3);

    for (let i = 0; i < scored.length; i++) {
      if (scored[i].score > 0) {
        await supabase.from("users").update({ badge: BADGES[i + 1] }).eq("id", scored[i].id);
      }
    }
